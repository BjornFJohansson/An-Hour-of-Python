...
# There are two ways to interact with python, interactively or by saving a text file with python code as a ".py" file. 
# Using python interactively will give us a response to each command. This is good for testing out python code lines, while writing and saving a
# python program ".py" file is good for 
# 
# Lets get used to the interpreter:
# An integer (datatype: int):

5

# A floating point number (datatype: float)
...
# value to a **variable**. The value of a variable 
# unitil you leave the Python interpreter.
...
# There are the aditional data types tuple and set, but we will leave them for
# now.
...
# We can delete a list element using "del"
...
def triangle_area(height, width):  # function name can have no spaces. height, width are arguments
    area = height * width / 2      # This is a **block** of code.
    return area                    # We want to return something from the function (area).


# Import libraries for more functions and other useful things
...
if 3<2:
    print("Three is smaller than two.")
